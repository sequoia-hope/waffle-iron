...
|---|---------|---------|------------|-------------|--------|
| 01 | kernel | Clean-sheet B-Rep geometry kernel | Rust | None | In progress (963 tests, 6 ignored; assay 190 cases) |
...
The clean-sheet kernel (`crates/kernel/`) is under active development. **963 kernel tests pass** (6 ignored). 190-case randomized assay corpus (seed 42) with analytical ground truth and Euler characteristic oracles. Assay score pending re-run (legacy S-H path; Yang path score is the target metric per A15.6).
