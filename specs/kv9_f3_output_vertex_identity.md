...
| E-V7 | **[MEASURED, NOT SHIPPED — ERROR-census campaign 5, 2026-07-08]** R0012's render-collapse wall: two pp-triple-point output vertices 7.1e-7 apart (sub-floor, A14.2 same-point) survive into a face loop and trip the always-on G1 render gate | The blocker is the merge CRITERION, not eligibility: the twins' incident triangles are LONG SLIVERS (7e-7 base × ~30 height ≈ 1e-5 area ≫ floor²), so the degenerate-triangle precondition never fires (an eligibility extension to `vert_pp_planes` members was tried and reverted — zero effect). Yang Fig. 11(b)'s own merge is VERTEX-DISTANCE-based; adopting it (any sub-floor edge merges) is exactly the P10-REVERTED global widening hazard at micro scale (the corpus carries genuinely-spread sub-floor edges at scale ~1e-4). UNBLOCK PATH: a scale-aware sub-feature floor (per-model relative floor, or the operand feature-size census) must be derived BEFORE the criterion can widen — or the twins' minting must be fixed at their source (measure which stage mints 7e-7 pp twins on R0012's gear flanks first). Tracker `pp_twin_merge_campaign.rs` stays `#[ignore]` RED |
