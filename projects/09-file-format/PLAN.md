...
- [ ] Phase 3b: KiCad board source — **spec written 2026-09-26, `specs/kicad_board_link.md`** (increments C1–C5; assessment `docs/kicad_board_link_assessment.md`). **C1 LANDED 2026-09-26** (`crates/kicad-pcb/`, 33 tests, fixture goldens); **C2 LANDED 2026-09-26** (`feature_engine::kicad`, bridge `ImportKicad`/`LinkKicadFromLocator`, 9 real-kernel tests; exact board volumes incl. rounded corners, slot and poly cutout); **C4 LANDED 2026-09-26** (`QueryEntityMeta`, MCP `kicad_link`/`entity_meta`, toolbar + Sources-panel link, hover card + detail panel, `kicad-link.spec.js`); next: C3 (step-import per-product split + board STEP components, O5 placement oracle — needs a `kicad-cli` export generated elsewhere) then C5 (re-sync of Derived content on update-to-tip)
