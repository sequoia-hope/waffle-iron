...
> **2026-09-21 (late) refresh.** Canonical corpus **295C / 0W / 11E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 810.0 s, F0085 332.0 s). **R0100 CONVERTED** — exactly one category move, zero detail moves. The §4.3.3 Case-IV rule-out (spec `yang_433_case_iv_corner_phantom.md` §8) now lives where the paper puts it: AFTER relocation, as a LOOP-level certificate — a closed cycle of A×B intersection edges every one of whose corners claims an edge pierce whose exact roots lie outside the edge's own segment is an intersection the surfaces do not have (Yang Fig. 8 Case IV; `refs/text:518-537`). Stage 4 STOPs typed `Stage4InvalidReason::PhantomIntersectionLoop` carrying the §4.5.2 under-resolution certificate (pierced owner's chord band over the claiming edge's certified in-band clearance), and the always-on op-level ladder refines and re-runs — the paper's own remedy for a detected local error (`:659-670`), which also creates the mirrored Case-III crossings §7 of the spec said a bare rule-out could not. R0100: natural STOP at v62, first rung d_ε/16 adopted (0 unpaired / 0 improper), SUPPORTED_CORRECT 1.3 s. The 2026-08-27 Stage-1 guard (refused: a density trigger over 26 cases, six CORRECT regressions) stays gated; the census population (R0004/R0011/R0044/R0049/R0053) is all CORRECT with no MIXED-loop fire. Remaining actionable tail: R0019, R0050, R0063, R0085.

> **2026-09-22 (night) refresh.** Canonical corpus **301C / 0W / 7E / 4EE / 0T + 0 UNSUPPORTED** (release, 8 jobs, 900 s; wall 907.0 s; F0085 359.7 s, F0072 517.0 s) — **F0072 CONVERTED; the ACTIONABLE ERROR TAIL IS EMPTY** (the seven ERROR rows are the loud-by-design set C0046, C0107, C0108, C0109, C0111, C0113, C0118). Exactly one category move, zero detail moves. The "Stage-5/6 non-2-manifold reassembly" wall was a non-manifold Stage-0 INPUT (`NONMANIFOLD_SITE_PROBE` ⇒ `i6-input-overuse`): the amendment-20 exact position oracle answered for a SLID circle∩edge crossing mint with the rational of its rounded projection — ~2e-17 off the OBLIQUE gear-flank line it had slid along (an axis-aligned line, F0064's, rounds onto itself) — so the amendment-6 joint region relocation ear-clipped a self-overlapping ring into four zero-area station fans and emitted them. Amendment 22 (spec `m8_stage0_multiclass_cavity_arm.md` §20): `ExactPos::host` — a crossing mint records the other input's exact edge it was minted on and a moved vertex with a host answers the exact foot on that line (`foot_on_line`); the ladder tries every split candidate of a folded triangle (`split_pairs`), and the slide splice reads positions through the oracle and re-selects C as the link edge on the host line that contains the mint. Sixteen `[fold-slide]` commits over op 11 (the four crossing mints of two tooth pairs among them). Four `host_line_tests` pins (RED/GREEN). Ledger `docs/yang_tail_triage.md` 2026-09-22 (night). Next: the loud-by-design rows (each a design call), torus × plane (M5 inc 3), SSI A15.4.

> **2026-09-22 (late night, later) refresh.** Canonical corpus **300C / 0W / 8E / 4EE / 0T + 0 UNSUPPORTED** (release, 8 jobs, 600 s; wall 828.1 s; F0085 340.1 s) — **F0064 CONVERTED, F0072 UNSUPPORTED → ERROR; the N17 `coplanar input face pair` `NotSupported` boundary is EMPTY on the corpus** (the UNSUPPORTED bucket is empty for the first time). Exactly two category moves, zero detail moves. Both walls were the same defect one layer past amendment 15: the open-link slide moved F0064's rim-chord ∩ cap-edge crossing mint (`q=12`) along the cap edge to the true circle ∩ line junction and re-fanned the overlay face coherently, but `collect_edge_splits` still ordered the PROPAGATED splits by their pre-relocation sweep parameter while storing the resolved point, so the mint's new position leapt past the sweep's event-column lifts on the same edge and the neighbour lateral's ring ran backwards (non-simple; `triangulate_ring` → None → the wall). The split table now keys on the exact projection parameter of the RESOLVED point along the 3D edge (`exact_edge_param`, rationals, a total order, no band) — spec `m8_stage0_multiclass_cavity_arm.md` §19 (amendment 21), killer RED/GREEN unit pin. F0072's next wall is a Stage-5/6 `non-2-manifold` reassembly STOP at op 11 (PROBE, ledger row). Ledger `docs/yang_tail_triage.md` 2026-09-22 (late night, later). Next: F0072's reassembly wall, the loud-by-design rows, torus × plane (M5 inc 3).

> **2026-09-22 (late night) refresh.** Canonical corpus **299C / 0W / 7E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 827.8 s) — **R0085 CONVERTED; the actionable ERROR tail is EMPTY** (the seven ERROR rows are the loud-by-design set C0046/C0107/C0108/C0109/C0111/C0113/C0118; the two UNSUPPORTED are the N17 coplanar pair). Three layers: (1) the 2026-09-18 "surface-pair samples bunched at a near-tangential crossing" STOP was a MISATTRIBUTION — the refused triangle spans three consecutive vertices of a 69-edge plane∩cone generator run, the raw CDT fans it legally, and kernel-v2's M1 grid-degeneracy flip pass MINTED the sub-resolution ear because its objective was the below-grid count (2 → 1); **M1c** scores flips by the lexicographic `(gate-refused, below-grid)` pair (spec `kv2_cdt_triangulation_core.md` §6e, killer RED on the count objective); (2) the op then graded SUPPORTED_WRONG on two non-manifold render edges — output B-Rep chains carrying the arrangement's grazing density (gear-edge split points 2e-5 apart with no intersection edge attached, pair-chain vertices 3e-7 apart) so both faces' CDTs formed the same ear; **Stage-5 §4.3.4 chain decimation** (`stage5_chain_decimate`, always-on, `YANG_434_DECIMATE=0|off`; spec `yang_434_output_chord_refinement.md`, deviation N58 addendum) drops intersection-run vertices the paper's own h/l/α acceptance calls redundant and the strict collinear sub-resolution subdivision points of straight operand edges — corpus-measured: only R0085 moved, zero detail moves; (3) the authored `euler_target` 2 was the generator's guess: the full-chain cubical ladder is out of scope at the 0.85° knife-edge plate × tube crossing, and Mayer–Vietoris on two STABLE readings (gear ∪ tube genus 1; plate ∩ body one ball) gives χ_solid 0 — the kernel's answer; corrected to 0, pinned (`assay_exact_membership::r0085_reads_genus_one_by_mayer_vietoris`). Ledger `docs/yang_tail_triage.md` 2026-09-22 (late night). Open PROBE: the origin of ruled-out crossing split points left on operand edges. Next: the loud-by-design rows, M8 coplanar Stage 0 (F0064/F0072), torus × plane (M5 inc 3).

> **2026-09-22 (night, later) refresh.** Canonical corpus **298C / 0W / 8E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 809.8 s, F0085 325.4 s) — **R0050 CONVERTED**. The §4.5.2 ladder's clause 4 reads the natural path's own acceptance again (a watertight 2-manifold emission; every downstream gate applies): the 2026-09-13 `improper == 0` demand on the Stage-4 mesh was justified only by kernel-v2's render gate refusing the same body while torus edges were `LineSegment` chord polylines, and the M5 torus arm removed that — R0050's d_ε/2 rung (0 unpaired, 57 exact chord contacts in the strip beside its 4.4° knife-edge void) is SUPPORTED_CORRECT on every oracle in 9.2 s. Strict clause = A/B knob `YANG_452_REQUIRE_CLEAN=1`. Spec `yang_452_local_refinement.md` §9; ledger row 2026-09-22 (night, later). Actionable tail: R0085 (its canonical sub-resolution cone-triangle wall, face 1761).

> **2026-09-22 (night) refresh.** Canonical corpus **297C / 0W / 9E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 823.8 s) — **R0019 CONVERTED**. **The M5 surface-pair curve has its TORUS arm** (spec `m5_surface_pair_curve.md` "Torus arm"): `Torus` joins the pair vocabulary in ssi-rs (`QuadricSurface::Torus`; perpendicular plane section → circles, every other torus pair → the `SurfacePair` descriptor), yang-rs (conversion / membership / tangent arms) and kernel-v2 (`PairSurface::Torus`: signed tube distance + unit gradient, band scale r, sag scale min(r, R−r)); torus × {cylinder, cone, sphere, torus} intersection edges now emit as `Curve::SurfacePair` by default (`YANG_TORUS_PAIR=0|off` is the A/B off-knob) — wall 1 of the 2026-09-22 (later) row is closed at the vocabulary, Stage 4 still relocates through the torus block, only the output tag moved. The arm-on measurement found three tag-visibility gaps, all fixed and A/B-certified: Stage 0's holed-lateral splice check refused any pair edge (R0026/R0051), and the §4.5.3 reversal sweep's cycle population lacked the typed pair edge (R0032's torus × cone chain kept eight zig-zag arrangement vertices; the same gap had left R0019's cylinder × cone chains unswept since the cone-pair producer landed — swept, its CDT ring-reject wall is gone). Torus × PLANE (spiric) stays a `LineSegment` (increment 3: a `Plane` pair operand + the K8 revision). Ledger `docs/yang_tail_triage.md` 2026-09-22 (night). Remaining actionable tail: R0050 (wall 2 — weld-band sampling of shared curved edges — now buildable), R0085.

> **2026-09-22 (later) refresh.** Canonical corpus UNCHANGED at **296C / 0W / 10E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 816.3 s; zero category and zero detail moves). **R0050 RE-DIAGNOSED — it is NOT a torus tangency**: for parallel-axis tori a tangent point must carry its normal along the axis or lie in the plane of both axes, and R0050's pair does neither; a direct scan puts the minimum dihedral along the intersection at 4.19° (the 09-13 "min |d| refines to 0" measured the transversal crossing itself). It is R0038's shallow-crossing class on tori. The §4.5.2 ladder's every rung failed ONE site type — B's torus piercing A's torus∩cap parallel circle at 2.0°, where the pair corridor mis-measured a 0.33 move ALONG the curve — so both Stage-4 triple gates gained the **three-slab junction bound** (`junction_slab_divergence`: the parallelepiped `|n_k·d| ≤ d_ε` on all three surfaces, of which the two line arms are special cases; never below the pair corridor; spec `yang_stage4_conic_triple_junction.md`). Every rung now completes but self-contacts in the strip along the 4.4° crossing and is not adopted; adopting it anyway meets kernel-v2's render self-intersection gate on a knife-edge VOID whose penetration is a boundary-chord-sag shadow, not a B-Rep defect. Two structural walls named, neither a band: (1) torus intersection curves leave Stage 3 as `LineSegment` chord polylines — the pair-surface vocabulary has no Torus although the P8 implicit-pair Newton needs only F + ∇F (owner: M5 surface-pair curve, torus arm; also a B-Rep/STEP fidelity item); (2) the render gate's band (1e-4·max_abs) sits below a shared curved boundary chord's render sag, so shared curved edges must be sampled to the WELD band — which needs (1) first. Also landed: kernel-v2's torus and sphere patch tessellators gather every boundary curve kind through one dispatcher (`boundary_half_edge_samples`; they took `Arc` samples only — the R0085 torus × cone T-junction class). Ledger `docs/yang_tail_triage.md` 2026-09-22 (later). Remaining actionable tail: R0019, R0050, R0085.

> **2026-09-22 refresh.** Canonical corpus **296C / 0W / 10E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 819.3 s; F0085 338.3 s, R0019 313.6 s, R0044 289.3 s). **R0063 CONVERTED** — exactly one move, zero detail moves (per-id diff of `results.json`). The 09-18 "rounding-luck latent" was located: KV14 Slice B's ribbon unroll opened each encircling loop into its u-ascending chain by anchoring at the min-u vertex and walking toward "whichever neighbour continues upward" — but R0063's op-2 holed lateral has a notch corner at the anchor whose GENERATOR-LINE wall rises from it at the same azimuth, so the two neighbours tied to within one ulp and the tie (which of the two corners the cap-normal rounding put lower) laid the chain DESCENDING: the ribbon ran the long way round at two heights and the chart polygon crossed itself (the loud STOP, demand `None` since no rim chord was involved). `open_chain` now reads the sense from the loop's WINDING (an exact ±2π quantity) and starts the chain just after its single seam wrap (the one large descending u-step); a loop whose neighbours differ is opened identically. Pin `yang-rs tests_unit/s1_ribbon_open_chain.rs` (a notched strip with an EXACT azimuth tie, both loop senses; RED on the old rule in the descending sense, certified by stash). Spec `yang_stage1_curved_holed_patch.md` "Slice B seam — R0063". Remaining actionable tail: R0019, R0050, R0085.
>
> **2026-09-21 (night) refresh.** Canonical corpus **294C / 0W / 12E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 817.7 s). **R0038 CONVERTED** — exactly one move, zero detail moves — and the Stage-4 `LocalRefinementRequired` family is EMPTY. R0038 was never the "plane tangent to a cylinder" the ledger and N2 spec §5c.10 recorded: measured from the document and the STOP probe's own positions it is A's outer cylinder (r 13.4185) crossing B's outer cylinder (r 15.2175) on parallel OBLIQUE revolve axes 1.93 apart at a 2.81° grazing angle. With parallel axes every facet-pair intersection is an axis-parallel line, so the arrangement returns one chord per crossing of the two cross-section polygons, and at a grazing angle the chord sags outrun the surface separation — three chords for one ruling, all relocated onto it, the strips between them collapsing into the zero-area chain. The §4.3.3 tangent mint gained its **CROSSING arm** (spec `yang_433_tangent_point_mesh_update.md` §13–§13.2, three checkpoints, each corpus-measured): (1) `cyl_cyl_crossing_generators` mints both crossing rulings of a parallel-axis pair through the §11 rim channel, and kernel-v2's planar-loop orientation oracle became an EXACT signed area (`geom::planar_loop_signed_area`, shoelace + closed-form arc segments) after the midpoint-sampled Newell polygon refused a CORRECT 0.10-thick crescent cap; (2) the SECTOR vocabulary (`[Arc, Line, Arc, Line]` laterals with per-arc containment) and the crossing-angle rim-density demand **`N ≥ π/α`** (the coordinate-frame replica showed a second polygon crossing 0.12° past the mint when A's uniform slot sat on its circle against B's 6.9° chord; demanded through `forced_rim_n`, ceiling 512); (3) the OBLIQUE frame — the overlap span's endpoints computed once and spliced into the longer operand's lateral as STANDING on-ruling face-interior points (`BRep::standing_face`, carried through every rebuild and Stage 0's emitted-mesh builds), so the shared segment is one bit-identical edge in both meshes. The answer is two bodies, the inner band holed through by op 3's torus (genus 1) and the untouched crescent: the meta's single `euler_target` could not express it, the cubical ladder cannot read the knife-edge crescent, so the coordinate-frame replica's full chain with a 3D grid volume oracle adjudicates and the meta authors `expected_shell_count: 2`. Remaining actionable tail: R0019, R0050, R0063, R0085, R0100. Sibling harness fix: `ModelBuilder::solid_handles` (a multi-body result's first body was all `solid_handle` returned).

> **2026-09-21 (later) refresh.** Canonical corpus **292C / 0W / 14E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 822.9 s; F0085 343.5 s) — ZERO moves, zero detail moves (per-id diff of `results.json`). **M8 mixed-loop coplanar caps RESOLVED** (the 09-19 note's item 2; spec `specs/m8_rim_override_provenance.md`): the mechanism was not "extend the disc path's exact projection to arcs" — it was three f64 spellings of ONE geometric split point (each cap's own overlay emission in its own frame + each one's f64 mirror onto the other cap's rim), kept apart by the bit-exact dedup; the bore's own full-circle rims carried the same latent (121 vs 124), the disc path only looked fine by bit-luck. `RimSplitMap` now carries PROVENANCE: a cap's OWN sample replaces a `TAU_WORK` near-twin MIRROR in place, a MIRROR is absorbed by a near OWN sample; own-vs-own and mirror-vs-mirror stay bit-exact (R0088/R0070 band-close twins, the #144 same-ray record). `sprocket_bore_with_coplanar_caps` un-quarantined; 8 stale `#[ignore = "… M8"]` quarantines (`cyl_cyl_cut_regression`, `extrude_on_extrude`, `boolean_shell_closure`) lifted — measured already passing. UNSUPPORTED stays F0064/F0072 (the N17 identical-overlap-mesh gap). The rewrite tier then flipped the `m8_disc_holed_polygon` §7 boundary pin `doubly_flush_crossing_stays_loud` (both tool caps flush with the plate's, rim crossing a hole edge — the same class): it now completes and grades CORRECT on the analytic volume, so it is the positive pair `doubly_flush_crossing_cut_succeeds` / `_union_succeeds`. Pre-existing and unrelated: `m8_samenormal_campaign::red_r0088_residual_pair` had been red since 0eabbb28 made R0088 EXPECTED_ERROR (consumed body); the pin now asserts the declared rebuild error.

> **2026-09-19 note (not a corpus refresh).** Two findings from landing the B3 sprocket entity (`PLAN.md` M14 in feature-engine): (1) kernel-v2 `exact2d`'s Tier-2 arc predicates lifted each arc's f64 centre/radius verbatim, so two arcs meeting TRANSVERSALLY at a shared vertex crossed exactly a few ulps off the corner, inside both open arcs about half the time — `ProfileNotSimple` by rounding luck and a SILENT fall-back to the 16-facet chord polygon (578 faces for a 9T sprocket). Every predicate now lifts to the rational circle through BOTH endpoints (centre snapped onto the chord bisector); integer fixtures are unchanged. (2) A through-bore whose caps are coplanar with a sprocket's STOPs in the M8 Stage-0 MIXED-loop path: `collect_mixed_crossings` mirrors each cap's overlay split points onto the opposite arc in f64, the mirrors are ULP-twins of the other cap's own points, the bit-exact dedup keeps both, the strip chains come out 15 vs 16 and the chart CDT sees exact duplicate `(u, v)` vertices. The disc path's exact opposite-rim projection / intra-opposite canonicalization needs extending to arcs. Pinned `#[ignore = "M8 …"] sprocket_bore_with_coplanar_caps` in `test-harness/tests/sprocket_kv2.rs`; details in feature-engine `PLAN.md` Blockers.

> **2026-09-13 (night, latest+2) refresh.** Canonical corpus **290C / 0W / 15E / 4EE / 0T + 3 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 745.8 s at host load < 1; F0085 328.3 s, R0044 294.7 s, F0065 111.2 s). **F0060 CONVERTED** — exactly one move, zero detail moves. The EDGE-pinch split (spec `yang_tangency_pinch_split.md` §0c; `YANG_EDGE_PINCH_SPLIT=0` is the off-switch) is ALWAYS-ON at Stage-4 entry. The "χ = 6 against authored 2" that kept it off was not a property of the output: yang emits `A − B` as FOUR closed shells of χ = 2 (12 faces; each lobe's tangent line its own edge, endpoints femto-twins from the arrangement; the tangent points as per-sheet vertex copies with identical bits — Mäntylä duplication, I6 verified: `from_yang` pairs by vertex index), and the assay χ oracle counted shells as VERTEX-connected components of the position-welded render, fusing each lobe pair at its tangent point (2 shells of χ = 3). The render's "twelve valence-1 edges" are the developable patch's designed zero-width chord-midpoint T-junctions, which the oracle already heals. Fix in `test-harness::oracle`: `shell_decomposition` — shells are EDGE-connected triangle components (crossing T-junctions through the subdivided cell keys), each welded vertex counted once per shell (`pinch_extra`); provably never demotes an old-correct verdict (old-correct ⇔ pinch copies = 2 × extra components); an edge shared by two sheets stays one non-manifold component and a self-pinch stays one χ short. Four oracle unit tests; F0060 smoke-pinned CORRECT. New `ASSAY_DUMP_OBJ=<dir>` per-face OBJ dump (docs/TESTING.md). Stage-5/6 non-2-manifold family: F0060 struck. Named, not fixed: a line pinch whose chain endpoints were bit-identical per sheet would still render as one 4-valent exact edge (no render-side per-sheet edge pairing); no corpus case exercises it.
> **2026-09-17 (night) refresh.** Canonical corpus **293C / 0W / 13E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 781.7 s; F0085 329.7 s, R0044 394.9 s, R0019 318.4 s). **R0070 CONVERTED** — exactly one move, zero detail moves. Two Stage-4 junction defects (spec `yang_stage4_conic_triple_junction.md`, the 2026-09-17 amendments): (1) the junction-line displacement metric gains its **LINE-CURVE arm** — the line the vertex moves along may be the vertex's exact Stage-3 Line curve (a plane∩cylinder generator, a Steinmetz line), not only the line of two planes; R0070's cap∩cut-cylinder generator pierced the boss lateral at |L̂·n| 0.345 and the triple block had gated the exact junction at the surface-pair corridor of two surfaces the move slides within neither of (`junction_line_curve_divergence`, certificate: on-line vertex, exactly two carriers, one transversal third; both triple arms, `YANG_JUNCTION_LINE=0|off` gate); (2) the **line × plane∩plane corner** (`vert_line ∩ vert_pp_planes`, the fifth junction map counting zero toward `n_maps`) is a triple-block candidate — before, the Line arm relocated it to the generator's perpendicular FOOT, on the line's carriers and off the third plane (Stage 6 `s6-planar-loop-nonplanar`, 8.5e-7). The evening entry's "region-boundary walk" reading of the wall was a misattribution (the STOP was the conic-endpoint `no_skip_audit` over two recorded §4.5.1 OffCurve failures). Pins: `tests_unit/s4_line_curve_junction_metric.rs`, kernel-v2 `tests/s4_line_pp_corner_chain.rs` (mutation-checked RED), smoke gate. Stage-4 `LocalRefinementRequired` family: R0038 alone.
> **2026-09-17 (evening) refresh.** Canonical corpus **292C / 0W / 14E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 772.1 s; F0085 321.2 s). **R0070 advanced UNSUPPORTED → ERROR** — exactly one move, zero detail moves. The M8 `disc-poly-holed` wall (a cut cylinder's disc on the holed planar cap a previous cut left behind) was a ROUTING gap: the disc-pair convex builder cannot express a hole, but the general §4.5.5 overlay already consumes a holed `PolygonWithHoles`; the tag now routes general like `disc-poly-nonconvex` (spec `m8_disc_holed_polygon_overlay.md`, 8 fixture tests incl. hole-edge crossing union/cut and a loud pin for the pre-existing doubly-flush-crossing boundary). R0070's next wall is Stage 4 `LocalRefinementRequired` at the region-boundary walk (§4.4.1 mesh-updating class). The UNSUPPORTED set is now F0064/F0072 alone — the N17 identical-overlap-mesh gap, the genuine §4.5.5 program.
>
> **2026-09-18 (later) refresh.** Canonical corpus **292C / 0W / 14E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 831.8 s). PLANE FIDELITY at both producers: `from_yang_brep` stores yang's STATED plane normal (the input's exact bits, inherited by Stage 5) instead of the loop's Newell recomputation, and the polygon extrude's caps carry `±profile.unit_normal()` exactly (the Euler operators' Newell tilts — 1.48e-15 on a gear cap — compounded through F0067's ten-extrude stack once the assembler was faithful). R0085's op-3 intra-solid pair is cleared; the op then STOPs loudly on a SUB-RESOLUTION cone triangle at a near-tangential torus × cone crossing (P10 gate = the corpus oracle's own rule: area < 1e-12 AND height < 4 f32 ulps; owner: pair-curve sample bunching + conformal cross-face boundary refinement). R0063 (gap at the feature-size floor) surfaces as a rounding-luck latent: CORRECT → loud Stage-1 chart self-crossing. A `RigidTwin` T-junction STOP was refuted on the corpus (33 CORRECT cases carry oracle-healed one-sided boundary splits). Two category moves, zero detail moves. Ledger `docs/yang_tail_triage.md` 2026-09-18 (later).
>
> **2026-09-18 refresh.** Canonical corpus **293C / 0W / 12E / 4EE / 0T + 3 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 760.2 s; F0085 330.8 s). R0085 op 2 CONVERTED by Yang §4.5.2: the §4-I9 STOP carries an UNDER-RESOLUTION certificate (max d_ε(far)/|d_far(q)| over every fire) that names the ladder's first rung (18.2 → d_ε/32; ceiling 64), and the §4.5.4 targeted retry now runs BEFORE the whole-op ladder (spec `specs/yang_452_local_refinement.md` §8). R0085 lands on op 3's INTRA-SOLID near-coplanar pair of the op-2 output (Stage-6 output-face plane fidelity — next wall). Exactly one category move.
>
> **2026-09-17 (later) refresh.** Canonical corpus **292C / 0W / 13E / 4EE / 0T + 3 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 776.9 s; F0085 327.9 s). **C0065 CONVERTED** — exactly one move, zero detail moves. **Yang §4.5.2 local refinement is now ALWAYS-ON** (`boolean::refine_452`, op-level d_ε/2 → d_ε/4 under the Q3 guard shell with clause 4 reading unpaired AND improper; spec `yang_452_local_refinement.md` §7, deviation N62): C0065 was its first MONOTONE customer — a shallow 14.8° crossing 0.05 deep under a 0.038 rim chord, the mesh loop never reaching the clip walls; the 2026-07-15 "refinement ⇒ silent-wrong" and 2026-08-29 "torus near-tangency" adjudications were both stale. The refined body's torus face (the closed tube minus two windows, no loop wrapping a period) then needed KV14 Slice F-4 in `tessellate_torus_patch` (`yang_stage1_curved_holed_patch.md`). `YANG_452_REFINE=0|off` restores the STOP-only behaviour. #137 is RESOLVED for C0065 by these two vehicles.
> **2026-09-17 refresh.** Canonical corpus **291C / 0W / 14E / 4EE / 0T + 3 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 738.1 s; F0085 321.6 s). **C0056 CONVERTED** — exactly one move, zero detail moves. The §4.3.3 tangent mint gained its GENERATOR arm (parallel-axis cylinder×cylinder tangent along a line: rim samples with identical non-axial bits, coordinate-axis-exact only; spec `yang_433_tangent_point_mesh_update.md` §11), and the line-pinched output then exposed three consumers: kernel-v2's pinch split now peels a 2-vertex sub-ring as a SLIT into an interior CDT constraint (M3d, `kv2_cdt_triangulation_core.md` §6d, new `cdt_polygon_with_holes_floodfill_constrained`), triangles on a spur node refine to half the facet width so the outer wall's render does not cross the hole wall's at the tangency (`SPUR_FACET_FRACTION`; not a band, the gate stays loud), and the assay χ oracle counts fused vertex copies per FAN rather than per touching shell (the 2026-09-13 "self-pinch stays one χ short" clause retracted). C0056's authored volume (full-height hole) corrected to 0.8π. Tangency family: C0056 struck; C0043 (coplanar caps) stays M8; R0038/R0050/C0065 unchanged.
