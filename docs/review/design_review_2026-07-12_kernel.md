...
| G6 | **FIP 5-role separation (Test Author ≠ Implementer, P5) is not followed** by the actual solo-agent increment workflow; adversary-validation docs partially honor the spirit. Also FIP §5.2 assumes coverage gates §12 lists as "planned". | FIP:34-43; Constitution P5; git log | **RESOLVED 2026-07-12 (user decision):** solo-operator variant amended into P5 + FIP §1 — roles may collapse while a frontier-class agent is the operator; phases and oracles do not |
| G7 | **P2 "spec in /specs/ precedes work" vs roadmap-as-plan-of-record ambiguity** — many Yang increments ship on roadmap milestones + memory entries without a per-increment spec; docs never say whether that satisfies or waives P2 | Constitution:52-58 vs CLAUDE.md/roadmap §0.1 | **RESOLVED 2026-07-12 (user decision):** P2 amended — roadmap milestone entry + mandatory yang_deviations.md entry IS the P2 artifact for Yang increments (deviations entry = merge blocker); full specs still required for user-facing features |
