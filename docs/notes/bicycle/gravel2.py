"""Gravel bike v2 for the Waffle Iron agent link — sprockets, pipe sweeps, patterns.

Emits, per part tab, an ordered list of MCP calls. "$SK" = the sketch feature id
returned by the sketch_create just before; "$F:<label>" = the feature id of an
earlier feature_add with that label (pattern seeds, pipe sketches).
