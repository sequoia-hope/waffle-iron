...
| Call | Records | Notes |
|---|---|---|
| `ctx.sketch(plane)` → sketch builder | a Sketch | `plane`: a `plane` parameter, a face query's `.plane_of()`, or `plane(origin, normal)`. A `#{ origin, normal, x_axis }` map (or a `plane` parameter given one) fixes the sketch's own +x direction, so the script does not have to reproduce the engine's derived basis to place anything oriented |
...
| `ctx.pattern_mirror(seed \| [seeds], #{ plane, combine?, targets? })` | a PatternMirror | `plane`: `#{ origin, direction }` where `direction` is the plane NORMAL, or a face/plane query; one copy (the seed's mirror image), so no `count` and no `skip` |
