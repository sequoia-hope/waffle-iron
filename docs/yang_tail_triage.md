...
1. **The STOP was misattributed.** `KV2_SUBRES_PROBE` on face 1761 (the
   89.15° plate cone): the refused triangle spans three CONSECUTIVE pool
   vertices of a 69-edge `LineSegment` run — the plane∩cone GENERATOR the
   arrangement split 69 times (2.6e-6 … 1.1e-2 apart; every edge
   `n_samples = 0`, so no pair sample was involved) — all at chart azimuth
   `u = ±1e-16`. The chart replayed offline (`KV2_SUBRES_DUMP`,
   `kernel-v2/tests/subres_dump_replay.rs`): spade's raw CDT fans the run
   legally from one apex beside its end; the M1 grid-degeneracy flip pass
   then swapped two flat-but-legal fans (6e-6 high at a 1.9e-5 grid — the
   chart is 67× anisotropic on this cone, so 2D-fat fans are 3D-flat) for
   the sub-resolution EAR over the collinear triple plus one above-grid
   triangle, because its objective was the below-grid COUNT (2 → 1). Fix
   **M1c** (spec `kv2_cdt_triangulation_core.md` §6e): acceptance is the
   lexicographic `(gate-refused, below-grid)` pair; killer
   `cdt_core_m1c_tests::m1c_flip_never_mints_a_subresolution_ear` (RED on
   the count objective: the planar G1 gate refused the fixture). R0085 op 3
   then COMPLETES and grades SUPPORTED_WRONG: `watertight_mesh: 2 unpaired
   edges (2 non-manifold)`.
2. **The WRONG was output density.** `ASSAY_WATERTIGHT_PROBE` + the OBJ
   census: both 4-use edges sit on op-2 gear cap∩flank edges whose split
   points are 2.1e-5 / 2.5e-5 apart with NO intersection edge attached
   (valence 2 on both faces — the crossings that made them were ruled out,
   the splits stayed), and BOTH faces' CDTs formed the ear over the same
   triple (a zero-thickness pleat). Every emitted intersection run carried
   the arrangement's grazing density too (a torus × cone `SurfacePair`
   chain with vertices 3e-7 apart). The paper's polylines are exactly as
   dense as §4.3.4 makes them (`:586-592`), so **Stage-5 chain decimation**
   (`stage5_chain_decimate`, after the I5-1b merge; spec
   `yang_434_output_chord_refinement.md` "Stage-5 chain decimation";
   deviation N58 addendum) drops the interior vertices of `LineSegment` /
   `SurfacePair` INTERSECTION runs the ratified h/l/α test calls redundant
   (twin-conformant per canonical chain, 3-edge loop floor, sources
   retagged to the covering piece) and the sub-resolution SUBDIVISION
   points of straight operand edges (strict: on the line through their
   kept neighbours to `TAU_WORK·(1+scale)`, within the same chord bound —
   an operand corner never qualifies). R0085: op 2 drops 12 / op 3 666
   vertices; watertight clears; the case grades `mesh_euler_characteristic
   … = 0 (expected 2)`. The first corpus (before the target correction):
   exactly ONE category move (R0085 ERROR → SUPPORTED_WRONG), ZERO detail
   moves — no other case's emission changed at all.
3. **The authored target was the generator's guess.** The full-chain
   cubical ladder is out of scope here (−2 / 2 / −2 / −2 at 384 … 1024
   cells, phase ½; 0 / 2 / 4 / −2 at phase ¼ — the 0.85° near-tangential
   plate × tube crossing is a knife edge the lattice perforates at every
   cell size). Adjudicated by Mayer–Vietoris on two STABLE readings
   (`assay_exact_membership::r0085_reads_genus_one_by_mayer_vietoris`,
   8 s): ops 1–2 (gear ∪ 289° tube) read boundary χ 0 / 1 component on
   six rungs (384/512/768 × two phases — the tube leaves the gear at 0° and
   re-enters at 180°), and the plate ∩ that body reads χ_solid 1 at 256 …
   1024 cells on both phases (its 1–10-cube extra components flicker with
   phase: the knife-edge slivers), so χ_solid(union) = 0 + 1 − 1 = 0 —
   boundary χ 0, the kernel's own answer; exact volume 59.25 (stable to
   0.05 % over the ladder) vs the kernel's 59.16. `euler_target` 2 → 0
   (meta description carries the derivation; pinned in
   `assay_euler_consistency::historical_authoring_fixes_pinned`).
   R0085: **SUPPORTED_CORRECT** (517.6 s release — too heavy for the smoke
   gate, which stays on R0050).

Open PROBE (not chased): why op 2's gear cap∩flank edge keeps its
ruled-out crossing split points (valence-2, 2e-5 apart). The decimation
removes them; their origin is a Stage-4/5 cleanup gap.

Corpus (release, 8 jobs, 600 s): **299C / 0W / 7E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** — per-id diff of the committed `results.json`: exactly ONE category move (R0085 ERROR → SUPPORTED_CORRECT), ZERO detail moves. wall 827.8 s. Actionable tail after this: EMPTY — the seven ERROR rows are the loud-by-design set (C0046, C0107, C0108, C0109, C0111, C0113, C0118) and the two UNSUPPORTED are the N17 coplanar pair (F0064, F0072).

## 2026-09-22 (night, later) — §4.5.2 clause 4 retires the `improper == 0` demand (the natural path's own acceptance; strict clause = A/B knob `YANG_452_REQUIRE_CLEAN`) — R0050 CONVERTS at d_ε/2; canonical **298C / 0W / 8E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)**

The arm-on note "the ladder apparently does not run on the typed edge" is
refuted by probe: on R0050 op 3 the ladder runs both rungs of the
certificate's `[2, 4]` and BOTH complete (0 unpaired; 57 / 25 improper
contacts) — the only thing refusing them was clause 4's `improper == 0`
demand (added 2026-09-13, `specs/yang_452_local_refinement.md` §7),
justified then solely by "kernel-v2's render gate refuses the same body one
crate later". That was true while the torus edge was a `LineSegment` chord
polyline; with the M5 torus arm the render sampler resamples the
`SurfacePair` edge and the d_ε/2 body is **SUPPORTED_CORRECT on every
oracle** (render selfx gate, watertight, χ, exact volume, composition).
The 57 contacts are the Stage-4 mesh's own chords crossing in the strip
beside the 4.4° knife-edge void (A:5 torus facet vs B:2 torus facet, the
2026-09-22 (later) row's census) — the tool's artifact, not the B-Rep's.
Clause 4 now reads the natural path's own acceptance (unpaired == 0; every
downstream gate still applies); the improper count is a probe / relative
measure again, as `output_improper_count`'s contract always said. Spec §9.
Corpus: release, 8 jobs, 600 s; wall 809.8 s, F0085 325.4 s — per-id diff of the committed `results.json`: exactly ONE category move (R0050 ERROR → SUPPORTED_CORRECT), ZERO detail moves. **Wall 2 of the 2026-09-22 (later) row
is CLOSED by the torus arm** — the shared curved edge is sampled by the
render sampler, not the Stage-4 chords. Actionable tail after this: R0085.
...
B's prism cap-corner wedge clears A's face-15 cone by 1.33 while the natural
mesh sags 2.29, the arrangement mints a 3-vertex loop [v62, v63, v64] the
surfaces do not have, relocation solves each corner to an exact-but-VIRTUAL
pierce (both roots of every edge×cone solve outside the edge's segment),
and the everted loop surfaced as a misnamed kernel-v2 face-15 CDT reject.
Both §7 refusals were about WHERE the certificate lives, and both dissolve
once it lives at the paper's own vantage:

1. **A Stage-1 density trigger cannot know whether a buried corner will
   mint a CLOSED junction loop** — 26 corpus cases share the geometry and
   most are CORRECT (six regressed under the gated guard). After
   relocation the question is exact: the inc-0 census's per-claim verdict
   (`classify_claims`, now shared) lifted to the loop — a connected
   component of A×B edges whose EVERY vertex is refuted (≥1 claim, all
   phantom), CLOSED within the refuted set and CYCLIC (degree ≥ 2, size
   ≥ 3). A MIXED loop is never certified (P10). The graph half is
   unit-tested (`closed_refuted_components`).
2. **"Ruling out leaves the B side bounded by phantom vertices whose
   missed Case-III crossings must be CREATED"** — that is Yang §4.5.2's job,
   not a mesh surgery: refine the surfaces of the erroneous region and
   re-run (`refs/text:659-670`). The op-level ladder has been always-on
   since 2026-09-17 and steered by the STOP's own under-resolution
   certificate since 2026-09-18 (R0085); the typed STOP
   `Stage4RegionInvalid { PhantomIntersectionLoop, under_resolution }` is
   its trigger. The certificate is `max d_ε(pierced owner) / g_edge` over
   the loop's claims, `g_edge` the certified lower bound on the claiming
   edge's distance to the pierced FACE (65 in-band samples minus the
   Lipschitz slack, the inc-1 guard's band superset) — in the ladder's own
   units (the operand-wide chord budget it divides): R0100 reads
   9.77 / 0.848 = 11.5 ⇒ first rung d_ε/16, A 156 → 604 tris, 0 unpaired /
   0 improper, adopted. SUPPORTED_CORRECT on all oracles in 1.3 s.

The §5 census population re-measured (the only cases that ever carried
phantom claims): R0004 3.6 s, R0011 1.8 s, R0044 263.2 s, R0049 1.6 s,
R0053 170.7 s — all SUPPORTED_CORRECT, no MIXED-loop certificate fires,
and the §7 R0011 silent-WRONG does not recur (the ladder adopts only on
the oracle). `YANG_433_RULEOUT=0|off` is the dev A/B off-knob; the inc-1
Stage-1 guard stays gated (`YANG_433_GUARD`), the census
(`YANG_433_PHANTOM`) now also prints the `[s433-ruleout]` verdict and each
phantom claim's clearance.

Corpus (release, 8 jobs, 600 s; wall 810.0 s, F0085 332.0 s): **295C / 0W /
11E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)** — per-id diff of the
committed `results.json`: exactly ONE category move (R0100 ERROR →
SUPPORTED_CORRECT), ZERO detail moves. Remaining actionable tail: R0019,
R0050, R0063, R0085. Smoke pin moved (`("R0100", SupportedCorrect)`).

## 2026-09-21 (night) — R0038 CONVERTED ⇒ 294C: the §4.3.3 crossing arm's checkpoint 3 (the OBLIQUE frame — the shared ruling segment made bit-identical in both meshes by STANDING on-ruling interior splices), and the answer adjudicated as TWO bodies, one holed by the torus cut; canonical 294C / 0W / 12E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)

Spec `yang_433_tangent_point_mesh_update.md` §13.2. Three things landed
over the (later) row's checkpoints 1–2:

1. **The oblique frame.** On R0038's axis `(0.4034, 0.9150, 0)` the four
   rim samples of the minted ruling are collinear only to rounding. The
   planner now computes the overlap span's endpoints ONCE (`S_lo`, `S_hi`);
   the operand whose rim bounds the overlap already has them as its rim
   samples (same expression, same bits), and the other operand gets them as
   face-interior points on its lateral, where the P3b inc-4e splice does
   the conforming 2+2 split of its ruling edge. `[S_lo, S_hi]` is then ONE
   identical edge in both meshes. Coordinate axes keep the rim-only path
   (byte-identical); a rim within 64 chord margins of a bound but not at it
   declines (ambiguous split).
2. **Standing face-interior points** (`BRep::standing_face`, the
   `standing_rim` analog) carried by every from-topology rebuild AND Stage
   0's emitted-mesh builds (`build_stage0_mesh`, the coincident-cylinder
   build via `stage1_tessellate_with_standing_overrides`). Without this the
   Stage-0 rebuild dropped the splices.
3. **The adjudication.** All three ops complete; the first grade was
   SUPPORTED_WRONG — "χ 2, expected 4 for 2 shells" — because the meta's
   single `euler_target: 2` decodes as ONE shell. The geometry (the output
   B-Rep, and by construction): op 3's torus tube (minor 1.55) crosses the
   0.13–0.35-thick inner band at A-azimuth ≈ 15°, mid-height, entering
   from B's inner wall and leaving into A's bore with its end cap in the
   void — a clean through-hole, genus 1 (inner loops on both cylinder walls
   + the torus face); the crescent (r ≥ 13.29) is beyond the torus's reach
   (12.81). Two shells, total χ = 2. The cubical ladder cannot read the
   document (knife-edge crescent: 7–8 components at 128–512 cells), so the
   coordinate-frame replica carries the full chain with a deterministic 3D
   grid volume oracle (`r0038_replica_full_chain_is_a_holed_band_and_a_
   crescent`: band χ 0, crescent χ 2, volumes to 2e-2 / 5e-3). Meta:
   `expected_shell_count: 2` (the R0003 precedent, strict).

Corpus (release, 8 jobs, 600 s; wall 817.7 s): **294C / 0W / 12E / 4EE /
0T, 2 UNSUPPORTED(coplanar-boolean)** — per-id diff of the committed
`results.json`: exactly ONE category move (R0038 ERROR →
SUPPORTED_CORRECT), ZERO detail moves. Smoke pin R0038 added. Remaining
actionable tail: R0019, R0050, R0063, R0085, R0100.
...
generalization to build (a rim-scan entry for the §4.5.1 corridor machinery
plus rider consumption). Measured under the corner probe, it is the paper's
Fig-13 EXCLUDED shape and the paper assigns it to §4.5.2 — and §4.5.2
resolves it (spec `specs/yang_452_local_refinement.md` §8):

1. **The site.** Gear extrude A ∪ torus B (R 1.5723 / r 1.0482, d_ε(far)
   2.6205e-2). The gear's tooth-root polyline at the crossed corner has rim
   edges 1.68e-3 … 1.85e-2, all shorter than the torus chord band; the exact
   torus∩cap curve exits the cap FIVE facets past the corner the mesh chain
   crossed (edge 232, t 0.354, 4.85e-2 from q) while the mesh chain hugs the
   rim just under the cap through six rider fires. Neither corrected triple
   is on a bounded face (`not-corner-incident` / `on-line-past-end`):
   `NoRealCandidate`, honestly — no transit exists to plan.
2. **The census ladder converges beyond the fixed budget.** d_ε/2 … d_ε/8
   reproduce `RelocationCrossedCarrierVertex` on the same chain, d_ε/16
   moves to an `OffCurveBeyondChordBand` rim junction on another tooth,
   **d_ε/32 converges** (b 312 → 9028 tris; 0 unpaired, 0 improper) and so
   does d_ε/64 — the monotone C0065 shape, not R0050's oscillation.
3. **The certificate names the rung.** The census `-RESOLUTION` verdict
   (`|d_far(q)| ≤ d_ε(far)` = UNDER-RESOLVED) is now a production quantity
   carried on the typed STOP (`Stage4RegionInvalid.under_resolution`,
   `under_resolution_ratio` over EVERY fire of the invocation — the STOP'd
   site reads 1.87 but the rider at the corner nearest the exact exit reads
   **18.19** with a 1.44e-3 clearance). `refine_452_rounds_for`: a demand
   above the fixed ladder's last rung starts the ladder at the first power
   of two strictly above it (18.19 → 32; d_ε/16 = 1.64e-3 is still over the
   clearance) and doubles to the new ceiling 64; a demand ≥ 64 yields an
   empty ladder (the STOP stands at once). No certificate, or a demand the
   fixed ladder already covers, keeps `[2, 4]` — R0038 (sentinel), R0050
   (3.38) and C0065 are byte-identical.
4. **Where it lands.** Op 2 completes (95 s release for the whole case); op 3
   (the rectangle revolve, `Revolve 3`) STOPs at Stage 0 on an
   **INTRA-SOLID near-coplanar pair of A** — `YANG_COPLANAR_PROBE`:
   `intra-solid input=A faces=(227,476)`, two planes equal to 1e-12 (the
   gear flank and a fragment of it that the op-2 output re-emitted with its
   own plane bits: the chained form errors.rs already names). NOT the F0064 /
   F0072 N17 cherchi deferral, and not an M8 overlay gap: a Stage-6 emission
   fidelity question (an output face must carry its INPUT's exact plane, one
   plane per input plane). Next wall, owner Stage 6.

Corpus (release, 8 jobs, 600 s; wall 760.2 s; F0085 330.8 s, R0019 317.7 s, R0044 294.0 s, R0085 162.4 s, F0065 111.4 s): **293C / 0W / 12E / 4EE / 0T, 3 UNSUPPORTED(coplanar-boolean)** — per-id diff of the committed `results.json`: exactly ONE category move (R0085 ERROR → UNSUPPORTED(coplanar-boolean)), ZERO detail moves. A first run with the ladder consulted BEFORE the §4.5.4 retry moved TWO (R0044 CORRECT → ERROR, `TessellationFailed FaceId(458)`) — spec §8.5; the reorder restores it. The Stage-4 `RelocationCrossedCarrierVertex` family is down to R0050 (exact tangency).

Pins: `yang-rs tests_unit/s452_under_resolution_ladder.rs` (R0085 numbers,
strictness at a power of two, empty ladder beyond the ceiling).

## 2026-09-17 (night) — R0070 CONVERTED ⇒ 293C: two Stage-4 junction defects — the junction-line metric gains its LINE-CURVE arm, and the line × plane-pair CORNER joins the triple block; canonical 293C / 0W / 13E / 4EE / 0T + 2 UNSUPPORTED(coplanar-boolean)

The "region-boundary walk" reading of R0070's Stage-4 wall (the evening
entry below) was wrong: `YANG_LRR_PROBE` names the site — `no_skip_audit`
(`stage4_correct.rs`, the conic-endpoint balance) firing because two
endpoints were RECORDED as §4.5.1 `OffCurveBeyondChordBand` failures at the
conic TRIPLE block's displacement gate (`[triple-gate]`, v88 / v476, a
mirrored pair). Anatomy (spec `yang_stage4_conic_triple_junction.md`, the two
2026-09-17 amendments):

1. **The generator pierce.** The cut cylinder's axis is parallel to the
   boss's annular cap, so cap ∩ cut-lateral is a GENERATOR line
   (`vert_line`); where the generator reaches the boss rim it pierces the
   boss lateral — {cap plane, boss lateral, cut lateral}. v88 sits EXACTLY
   on the generator (cap residual −3.5e-18, cut residual 0), 6.116e-4 inside
   the boss lateral (d_ε 7.3383e-4), and the exact junction lies 2.0014e-3
   ALONG the generator (zero off-line component) at |L̂·n_boss| = 0.3454.
   The gate measured the surface-pair corridor between the cap and the
   boss lateral — sin θ = 1, two surfaces the move slides within NEITHER
   of — 1.4677e-3, and refused; `junction_line_divergence` (the R0077
   two-plane line metric) declined because only ONE surface is a plane.
   The §4.5.2 ladder could not rescue it: the corridor halves with d_ε while
   the along-line offset shrinks no faster (rung 3: 4.54e-4 vs 3.67e-4).
   Fix: `stage4_relocate::junction_line_curve_divergence` — the line
   supplied by the vertex's exact Line curve, with a certificate (vertex on
   the line to 1e-9·(1+‖p‖); exactly two carriers, normals ⊥ L̂; one
   transversal third) — wired at both triple arms as the two-plane helper's
   fallback (probe `metric=line-curve`, gate 4.249e-3). Pin
   `tests_unit/s4_line_curve_junction_metric.rs`.
2. **The line × plane-pair corner** (the wall behind it: Stage 6
   `s6-planar-loop-nonplanar`, face 188 — a gear-flank plane — vertex
   8.455e-7 off-plane against 1.022e-7). The bottom∩boss-lateral generator
   ENDS where it meets the flank: a `vert_line` endpoint that also
   terminates the bottom∩flank plane∩plane segment (`vert_pp_planes`),
   sitting BEFORE Stage 4 exactly on both planes (9.3e-17 / −1.7e-18) and
   3.75e-6 inside the lateral. `vert_pp_planes` is the FIFTH map counting
   zero toward `n_maps` (after KV16 same-type, R0044 pair, M5 K11
   line×circle, C0067 circle-pair), so the vertex skipped the triple block
   and the Line arm relocated it to the generator's perpendicular FOOT — on
   the line's two carriers, off the third plane by the foot's along-line
   error (the wrong point of the right line; a sub-band foot would have
   passed Stage 6 silently). Fix: `pp_line_corner` joins the `n_maps < 2`
   bypass; the 3-surface Newton lands it (ρ 4.12e-6, line-corridor gate
   1.61e-3). Pin: kernel-v2 `tests/s4_line_pp_corner_chain.rs` (z-boss,
   TILTED slot, x-drilled hole; three flank corners + three generator
   pierces exact; RED `non-2-manifold` without the candidate,
   mutation-checked).

Corpus (release, 8 jobs, 600 s; wall 781.7 s; F0085 329.7 s, R0044 394.9 s,
R0019 318.4 s, F0065 112.3 s): **293C / 0W / 13E / 4EE / 0T, 2
UNSUPPORTED(coplanar-boolean)** — per-id diff of the committed
`results.json`: exactly ONE category move (R0070 ERROR → SUPPORTED_CORRECT,
14.0 s), ZERO detail moves. R0070 struck (row below). The Stage-4 LRR
family is down to R0038 (tangency); RelocationCrossedCarrierVertex
R0050/R0085 unchanged.
...
established the mechanism: two equal-radius cylinders whose axes cross touch at
two isolated points, the band between the section curves pinches to ZERO width
there, and the inscribed prisms therefore do not meet at all — at Stage-4 entry
NO mesh vertex lies within 1e-9 of the tangent point. Relocation moves geometry,
not CONNECTIVITY, so A's kept region stayed edge-connected across the band and
Stage 6 saw a Newell-cancelling 64-edge cycle (C0058) / a four-triangle edge
(F0058). No mesh resolution resolves a zero-width band, so refinement was never
the route (and yang2023 §5.4 certifies it does not converge near tangency).

**The fix is Yang's own sentence.** §4.4.1: *"we trim and update the meshes using
the intersection curves … **Then we set r_A = r_B = r**, so that the two
polylines in the meshes coincide with the intersection curve"* (`:552-570`), with
§4.3.3's collinear-normal test making the tangent point first class (`:518-570`).
The general pre-boolean re-trim is deviation N2's architectural closure and stays
out of scope — but the half this class needs can be done where the pipeline still
has freedom, at **Stage 1**: mint the exact tangent point into BOTH operands'
tessellations, with identical bits. Both inscribed surfaces then fall away from
that shared apex in the same normal direction with different second-order forms
(`d_A²/2r` vs `d_B²/2r`), so their intersection near the apex is the four rays
`|d_A| = |d_B|` — the four alternating A,B,A,B sectors the exact geometry has.
Nothing downstream needs a special case: A's kept region separates by ordinary
flood fill and both boundary cycles come out simple.

**`boolean::tangency` (new).** `cyl_cyl_tangent_points` is a closed form: a
cylinder's normal is radial, hence ⊥ to its axis, so a shared normal must be
`m = ±(û × v̂)/|û × v̂|`; expanding a candidate in `{û, v̂, m}` anchored at the
common perpendicular's foot `f_A` kills both axial components and leaves
`p = f_A + s_A·R_A·m`, admissible **iff** `s_A·R_A − s_B·R_B = δ` with
`δ = (b − a)·m` the signed axis offset. Equal radii + intersecting axes (δ = 0)
⇒ the TWO Steinmetz points; `δ = R_A − R_B` ⇒ one; unequal radii + intersecting
axes ⇒ none; parallel axes ⇒ `None` (generator tangency — the F0060 vehicle).
The mint uses BOTH Stage-1 channels and both are load-bearing: the face
interior carries the point, and rim samples carry its AZIMUTH onto the tube's two
rim rings so the grid has a full RULING through it and the interior splice lands
ON that ruling (a conforming 2+2 edge split) instead of fanning a mid-quad
Steiner point into three slivers. Measured face-channel-only: an operand whose
seam phase puts the tangency mid-quad produced an arrangement edge **1.35e-1 from
one exact branch and 1.88e-1 from the other** — on neither — and Stage 3 refused
it loudly (`AmbiguousCurve { candidates: 2, matched: 2 }`, both matching only
because `cyl_cyl_point_amplification` is unbounded at tangency grade, the same
weakness the morning's Stage-4 fix closed). `tangent_point_face_overrides` gates
each pair fail-closed: the canonical-TUBE
vocabulary `line_edge_cylinder_face_pierce` already uses (hole-free, outer loop
= exactly two full-circle rims, so axial containment is exact via the rim
planes); the tangency identity within the **`TAU_WORK·(1+scale)` ROUNDING band**
— never `TAU_MODEL`, which would fuse a real sub-resolution gap into a tangency
(R0053); an on-surface postcondition at `TAU_EVAL·(1+scale)`; and strict axial
containment inside BOTH tubes with the `TAU_MODEL·(1+scale)` rim margin. The
points ride the EXISTING P3a #146 / P3b inc-2 face-interior channel
(`junction_stage1_overrides` → `rebuilt_with_all_overrides` →
`splice_lateral_interior_points`, which consumes each as a 3-fan, a 2+2 edge
split if it lands on a ruling, or a loud error) and are registered in
`minted_junction_keys` like every other Stage-1 mint. A tangency is not a pierce
— no edge crosses a face — so it has no entry in `pierce` and the P3a arms
cannot see it; it is minted on the SURFACES alone.

Corpus (release, 8 jobs, 600 s; wall 762.0 s at host load ≈ 7): **289C / 0W /
16E / 4EE / 0T, 3 UNSUPPORTED(coplanar-boolean)** — exactly TWO category moves
(C0058, F0058, both ERROR → SUPPORTED_CORRECT) and **ZERO detail moves**, so no
CORRECT case sees a different mesh. Solo: C0058 28.0 s, F0058 0.6 s release.
ALWAYS-ON; `YANG_433_TANGENT_INSERT=off|0` is the dev A/B disable. Both KV9-F1
steinmetz E2E oracles un-quarantined — the union's analytic
`V = 2·πr²h − 16r³/3` is RED gate-OFF (mutation-checked), while its subtract
twin passes gate-OFF too and is recorded as a STALE quarantine, not a
conversion. Smoke pin: F0058 (0.6 s); C0058 deliberately NOT pinned (28.0 s
release — the R0044 / F0082 debug-ratio rule), its conversion carried by the
committed corpus snapshot and the kv9 union oracle. Pins:
`tests_unit::s433_tangent_relocation` (10). **One cost, recorded:**
`tangency_pinch_split.rs`'s 30° symmetric fixture (r 0.4, h 4.0, hand-built
`cylinder_brep`) is QUARANTINED — the four sheets that now meet at its tangency
defeat the Stage-6 boundary walk (`s6-wedge-walk-not-outgoing` at vertex 44: one
wedge with BOTH terminal edges incoming, the legacy fallback also
`NonManifoldOutput`), the limitation `patch_boundary_cycle` already names as
awaiting a curvature-aware radial sort, now REACHABLE where the un-resolved
tangency kept it out of reach. Its C0058-authored twin passes a strictly
stronger oracle (no coincident-position vertex group at all). Net coverage rises:
two kernel-v2 E2E oracles un-quarantined against one fixture parked. Remaining
actionable tail: **7**
(R0038, R0050, R0100, F0060, C0065, R0019, R0085); loud by design: 9.
...
| ~~R0038~~ | ~~Stage-4 LRR (u32::MAX)~~ **CONVERTED 2026-09-21 (night), spec §13–§13.2** | ~~plane tangent to cylinder along one generator~~ **RE-DIAGNOSED 2026-09-21 (later): A's outer cylinder r 13.4185 × B's outer cylinder r 15.2175, parallel OBLIQUE revolve axes 1.9303 apart, CROSSING along one mid-sector ruling at 2.81° — three arrangement chords for one exact ruling collapse under relocation. Owner: §4.3.3 §13 crossing-ruling mint — checkpoint 1 (coordinate-axis full tubes), 2 (sector vocabulary + `N ≥ π/α` rim demand), 3 (oblique frame: bit-identical shared segment via standing on-ruling interior splices). The answer is two bodies, one holed by op 3's torus: authored `expected_shell_count: 2`.** | CONFIRMED (#168 WIP4, 9f4cb604) | §4.3.3 §13 |
