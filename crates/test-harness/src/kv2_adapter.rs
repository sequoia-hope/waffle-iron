...
impl KernelV2Adapter {
    ...
    fn face_signature(&self, fid: FaceId) -> TopoSignature {
        ...
        for (li, lid) in loops.iter().enumerate() {
            ...
            let nw = kernel_v2::geom::newell(&pts);
    ...
    fn run_boolean(
        ...
        match kernel_v2::boolean_op(&mut self.arena, sa, sb, op) {
...
impl Kernel for KernelV2Adapter {
    ...
    fn tessellate(
        ...
        kernel_v2::validate_solid(&self.arena, sid).map_err(|e| KernelError::Other {
            message: format!("kernel-v2 validate_solid failed before tessellation: {e}"),
        })?;
        let mesh = kernel_v2::tessellate(&self.arena, sid).map_err(|e| {
    ...
    fn make_faces_from_profiles(
        ...
        for profile in profiles {
            ...
            let kv2_profile = kernel_v2::Profile::new(
