//! Pattern instancing: rigid copies of seed bodies at a list of placements
...
//! body through `Kernel::transform_body`, which is exact on analytic
//! geometry. Instance 0 is the seed body itself (identity placement — the
...
use waffle_types::kernel::{KernelSolidHandle, RigidPlacement};
...
/// Instance `seeds` at `placements`. `placements[0]` must be the identity
/// (the seed is re-emitted, not copied); every later placement produces one
/// copy per seed. Outputs are ordered instance-major: for placement `i`, the
/// seeds in order — so with S seeds the body for (instance i, seed s) is
/// output `i·S + s`; output 0 is `Main`, the rest `Body { index }`.
///
...
pub fn execute_pattern_instances(
    ...
    if placements[0] != RigidPlacement::IDENTITY {
        return Err(OpError::InvalidParameter {
            reason: "pattern instance 0 must be the identity placement (the seed itself)".into(),
        });
    }
    for &s in skip {
        if s == 0 {
