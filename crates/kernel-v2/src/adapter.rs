...
//! | `revolve_face` | SUPPORTED (PR-KV6a; full-turn cones PR-KV6c) | staged polygon profile → `kernel_v2::revolve` (degrees → radians; world-space in-plane axis). Oblique edges sweep `Surface::Cone` frustum bands on a FULL-turn revolve (KV6c); a PARTIAL-turn oblique edge (arc-bounded cone patch, KV6c increment 5) → `NotSupported`. A PARTIAL-turn circle profile sweeps a `Surface::Torus` (a bent solid tube, KV6d); a FULL-turn circle profile (a closed genus-1 torus) → `NotSupported`. Typed walls: holed profiles → `NotSupported`; axis touching/crossing the profile and out-of-range angles → `KernelError::Other` (INVALID INPUT — the F0073/F0074 expected-rebuild-error path, never the NotSupported marker) |
    ...
    fn revolve_face(
        ...
        .map_err(|e| match e {
            ...
            KernelV2Error::RevolveObliqueEdgeUnsupported => Self::not_supported(
                "revolve_face: partial revolve of an oblique edge sweeps an arc-bounded CONE patch (kernel-v2 roadmap KV6c increment 5; full-turn cones are supported)",
            ),
