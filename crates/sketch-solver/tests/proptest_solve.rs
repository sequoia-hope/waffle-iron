...
fn dist(positions: &std::collections::HashMap<PointId, (f64, f64)>, a: PointId, b: PointId) -> f64 {
...
proptest! {
    ...
    fn proptest_rectangle_perturbed(
        ...
        let (_, y1) = result.positions[&PointId(1)];
        let (_, y2) = result.positions[&PointId(2)];
    ...
    fn proptest_triangle_exact(
        ...
        let d20 = dist(&result.positions, PointId(3), PointId(1));
    ...
    fn proptest_polygon_5(
        ...
        for (i, &expected_d) in distances.iter().enumerate() {
            let j = (i + 1) % 5;
            let d = dist(&result.positions, PointId((i + 1) as u32), PointId((j + 1) as u32));
            prop_assert!(
